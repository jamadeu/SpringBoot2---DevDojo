...
class AnimeControllerIT {
    ...
    static class Config {
        ...
        public TestRestTemplate testRestTemplateRoleAdminCreator(@Value("${local.server.port}") int port) {
            RestTemplateBuilder restTemplateBuilder = new RestTemplateBuilder()
                    .rootUri("http://localhost:" + port)
                    .basicAuthentication("jean", "academy");
            return new TestRestTemplate(restTemplateBuilder);
        }
    }
    ...
    void findByName_ReturnsAnEmptyListOfAnimes_WhenAnimeIsNotFound() {
        devDojoUserRepository.save(USER);
        List<Anime> animes = testRestTemplateRoleUser.exchange("/animes/find?name=animeNotFound", HttpMethod.GET, null,
                new ParameterizedTypeReference<List<Anime>>() {
                }).getBody();
    ...
    void save_ReturnsAnime_WhenSuccessful() {
        NewAnimeRequest newAnimeRequest = NewAnimeRequestCreator.createNewAnimeRequest();
        devDojoUserRepository.save(USER);
        ResponseEntity<Anime> animeResponseEntity = testRestTemplateRoleUser.postForEntity("/animes", newAnimeRequest, Anime.class);
